...
test("check author and title is rendered correctly", () => {
  ...
  const { container } = render(
    <Blog
      blog={blog}
      deleteBlogHandler={mockDeleteBlogHandler}
      likeBlogHandler={mockLikeBlogHandler}
    />
  );
  ...
  expect(blogText).toHaveTextContent(
    /Component testing is done with react-testing-library J. Smith 1234567890/
  );
...
test("test the like button works (twice)", async () => {
  ...
  const mockLikeBlogHandler = jest.fn().mockImplementation(() => {
    likes = likes + 1;
